...
fn main() {
    ...
    let ll = format!("{}/transpose.ll", out);
    let obj = format!("{}/transpose.o", out);
    let s = Command::new("racket")
        .args(["-l","wyve/cli","--","build",&format!("{}/transpose.wyv",here),"-o",&ll])
    ...
    Command::new(ar).args(["crs",&format!("{}/libtranspose.a",out),&obj]).status().unwrap();
    println!("cargo:rustc-link-search=native={}", out);
    println!("cargo:rustc-link-lib=static=transpose");
    println!("cargo:rerun-if-changed={}/transpose.wyv", here);
}
