...
(define (emit-module source-name kernels)
  (define md '())     ; list of (id . width)
  (define md-next 0)
  (define (md-alloc! width)
    (define id md-next)
    (set! md-next (+ md-next (if width 3 2)))
    (set! md (append md (list (cons id width))))
    id)
  ...
  (unless (null? md)
    ...
    (for ([entry (in-list md)])
      ...
      (cond
        [w
         (fprintf o "!~a = distinct !{!~a, !~a, !~a}\n" id id (+ id 1) (+ id 2))
         (fprintf o "!~a = !{!\"llvm.loop.vectorize.enable\", i1 true}\n" (+ id 1))
         (fprintf o "!~a = !{!\"llvm.loop.vectorize.width\", i32 ~a}\n" (+ id 2) w)]
        [else
         (fprintf o "!~a = distinct !{!~a, !~a}\n" id id (+ id 1))
         (fprintf o "!~a = !{!\"llvm.loop.vectorize.enable\", i1 true}\n" (+ id 1))])))
  (get-output-string o))
...
(define (emit-kernel o k md-alloc!)
  ...
  (define vmd (Contracts-vectorize (Sig-contracts decl)))
  ...
  (define (st s)
    (match s
      ...
      [(SFor var init cond-e body _)
       ...
       (define md-ref
         (if vmd
             (format ", !llvm.loop !~a" (md-alloc! (Vectorize-width vmd)))
             ""))
